...
-- 4
select hire_date, concat(first_name,' ',last_name) as full_name, department_id from employees where job_id like '%clerk';
-- 5
...
-- 10
select concat(first_name, ' ',last_name) as ename, department_id as dept_no from employees where commission_pct is not null order by salary desc;
-- 11
 select e.last_name, j.job_title from employees e, jobs j where 
...
-- Page 30                
-- 1
select employee_id, avg(salary), max(salary), min(salary), commission_pct from employees where commission_pct is not null group by employee_id;

-- 2
select department_id, sum(salary) as total_salary, sum(commission_pct) as total_commision from employees group by department_id;
...
-- 5
select * from employees;
select employee_id, commission_pct from employees where commission_pct is null order by 1;
...
-- 7
select concat(first_name,' ',last_name) as employee_name, salary, (2*commission_pct) as rise_commission, commission_val from employees;

-- 8
select concat(e.first_name, ' ',e.last_name) as employee_name, d.department_id from employees e join employees d on e.first_name = d.first_name;
...
-- 10
use hr_a;
select m.first_name as manager_name, count(m.employee_id) as employee_count, m.department_id from employees e join employees m on e.manager_id = m.employee_id where
	e.department_id = m.department_id group by m.first_name, m.department_id; -- issue with department

-- 11
select e.first_name as employee_name, e.department_id, e.salary from employees e join employees m on e.manager_id = m.employee_id where
	e.last_name like '_a%';

-- 12
select avg(salary) as Avg_salary, sum(salary) from employees group by department_id order by department_id;
...
select employee_id,commission_pct,
	case 
        ...
        end new_commission from employees;
        
        
-- Page 31
-- 1
select employee_id, concat(first_name,' ',upper(substr(last_name,1,1)), lower(substr(last_name,2,4))) as E_name from employees;
...
-- 4
select e.employee_id, 
concat(substring(e.employee_id, 1,2), '00',
...
-- 5 (Length part)
select case 
	when substr(first_name, 1, 1) in ('J','A','M') then concat(first_name, ' ', upper(substr(last_name,1,1)), lower(substr(last_name,2)))
    else concat(first_name, ' ', lower(last_name))
    end New_name,
...
-- 10
select first_name, lower(concat(substr(first_name,1,1),last_name,"@systechusa.com")) as Email_address from employees;

-- 11
select concat(e.first_name, ' ',e.last_name) as employee_name, j.job_title from employees e join jobs j on e.job_id = j.job_id where j.job_title = 
...
select e.first_name, d.department_name from employees e join departments d on e.department_id = d.department_id 
	...
	join employees e2 on e2.department_id=d2.department_id where 
	e2.first_name like 'Trenna');
...
select * from job_history; -- where department_id = 40;
select j.job_id, l.location_id, l.city, d.department_id from departments d join job_history j on d.department_id = j.department_id
join locations l 
...
join locations l on d.location_id = l.location_id where
		e.commission_pct is not null;
        
-- 4
select concat(e.first_name, ' ', e.last_name) as employee_name, d.department_name from employees e join departments d on 
	e.department_id = d.department_id where e.last_name like '%a%';
    
-- 5
select * from locations;
select * from departments;
select e.last_name, j.job_title, jh.department_id, d.department_name, l.city from employees e join departments d on e.department_id=d.department_id
	join job_history jh on e.department_id = jh.department_id 
    join jobs j on jh.job_id = j.job_id
    join locations l on d.location_id=l.location_id
    where l.city like 'ATLANTA';
    
-- 6
select e.last_name as Employee_lastname, e.employee_id as Employee_id, m.last_name as Manager_lastname, m.employee_id as Manager_id from employees e 
...
-- 8
select * from employees;
select last_name as employee_lastname, department_id from employees where department_id = 
	(select department_id from employees where last_name = 'bull');
...
select concat(e.first_name, ' ',e.last_name) as employee_name, d.department_name, e.salary, 
    ...
    when salary >= 13500 and salary <20000 then 'B'
    when salary < 13500 then 'C'
    else 'No grade'
    end Grade
...
-- page 34
-- 1
select e.last_name, e.hire_date, d.department_name from employees e join departments d on e.department_id = d.department_id where e.last_name in	
	(select e.last_name from employees where d.department_name like '%Sales');
...
-- 4
select e.employee_id, d.department_id, j.job_id from employees e join departments d on e.department_id = d.department_id 
join  jobs j on e.job_id = j.job_id 
join locations l on d.location_id = l.location_id
where l.city like 'ATLANTA';
    
-- 5
select last_name, salary from employees where manager_id = (select employee_id from employees where last_name like 'FILLMORE');

-- 6
select e.department_id, e.last_name, e.job_id from employees e join departments d on e.department_id = d.department_id where 
	d.department_name like 'OPERATIONS';

-- 7
select * from employees;
select employee_id, last_name, salary from employees where department_id 
	in (select department_id from employees where last_name like '%u%') and salary > (select avg(salary) from employees);
    
-- 8
        select * from employees;
 select e.first_name, j.job_title from employees e join jobs j on e.job_id = j.job_id where 
	j.job_title in (select distinct jt.job_title from jobs jt join employees em on jt.job_id = em.job_id join departments d on 
...
-- 11
select concat(first_name, ' ',last_name) as Name, salary, case
...
-- 12
select concat(first_name, ' ', last_name) as Name, department_id,salary from employees where employee_id in 
...
INSERT INTO orders (order_id, customer_id, order_date, total_amount,DELIVERY_TINE)
VALUES
    (1, 101, '2023-01-15 08:30:00', 150.00, '00:30:03'),
    (2, 102, '2023-02-20 14:45:00', 220.50, '05:30:03'),
    (3, 103, '2024-06-07 10:15:00', 75.25, '03:40:03'),
    (4, 101, '2024-06-08 16:00:00', 100.00, '04:30:03'),
    (5, 104, '2024-6-11 09:30:00', 300.75, '02:10:03'),
    (6, 102, '2023-05-20 11:20:00', 180.60, '04:30:03'),
    (7, 106, '2023-05-25 13:45:00', 90.40, '03:45:02');
    
select * from orders;

-- page 35
-- 1
select employee_id, concat(extract(day from end_date - start_date), ' Days ', extract(month from end_date - start_date), ' Months ',
	extract(year from end_date - start_date) - 2000 , ' Year ') as Duration from job_history; 
    
-- 2
select employee_id,date_format(start_date, '%m/%d/%Y') as hire_date, case
	when start_date is null and end_date is null then 'Dec, 01th 1900'
    else date_format(end_date, '%m/%d/%Y')
    end Resignation_date
    from job_history;
    
-- 3
select employee_id, concat(extract(year from end_date - start_date) - 2000 , ' Year ', 
	extract(month from end_date - start_date), ' Months ') as Experience from job_history; 
    
-- 4 
select current_date(), case 
	when extract(year from current_date())/4 = 0 then "Not Leap year"
    else "Leap year"
    end Type_of_year;
